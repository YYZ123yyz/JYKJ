...
public class AppointOrderDetialPresenter
    ...
    @Override
    public void sendSearchReserveInfoRequest(String orderType, String orderCode, Activity activity) {
        HashMap<String, Object> hashMap = ParameUtil.buildBaseDoctorParam(activity);
        hashMap.put("orderType",orderType);
        hashMap.put("orderCode",orderCode);
        String s = RetrofitUtil.encodeParam(hashMap);
        ApiHelper.getApiService().searchReserveInfo(s).compose(Transformer.switchSchedulers(new ILoadingView() {
            @Override
